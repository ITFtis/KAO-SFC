        ...
        initUi: function () {
            ...
            this.$pinctrl = $('<div>').appendTo(this.$element).on($.BasePinCtrl.eventKeys.initUICompleted, function () {
                ...
                $districtSelect.on('change', function () {
                    ...
                    if (_sidx) {
                        that.$pinctrl.PinCtrl('setBoundary', $districtSelect.find('option:eq(' + _sidx + ')')[0].coors);
                    }
                    else
                        that.$pinctrl.PinCtrl('setBoundary', undefined); //資料已過濾剩高雄市
                    //boundary.helper.FindUseBoundary(boundary.data.County, ["高雄市"], function (bs) {
                    //    that.$pinctrl.PinCtrl('setBoundary', bs[0].coors);
                    //});
